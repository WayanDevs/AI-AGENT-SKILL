...
5. **Offload KV Cache to CPU (LM Studio / Local Servers)**:
For massive context windows, a small GPU VRAM (like 4GB) will easily encounter CUDA allocation errors during KV Cache allocations. Force the server to run fully on CPU using `--gpu off` and lower context settings (e.g. 128k/64k instead of 200k) to stay within physical RAM budgets. You can load this via LM Studio CLI (`lms`):
```bash
lms load <model-key> --gpu off --context-length 128000 -y
```
