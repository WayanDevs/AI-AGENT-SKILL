...
**9Router integration:** When using 9Router (https://9router.com) as a local AI gateway, query available models via the OpenAI-compatible endpoint. See `references/9router-model-discovery.md` for model discovery patterns when the dashboard is inaccessible.
...
- **Desktop launcher fails from GNOME menu**: PATH mismatch — terminal has NVM/pyenv paths, GNOME doesn't. Hardcode PATH in launcher script. See `references/desktop-launcher-path-fix.md`.
- **LM Studio empty stream error**: Reasoning models (Qwen3.5, o1/o3) send `reasoning_content` before `content`, causing Hermes timeout. Disable reasoning in LM Studio or use non-reasoning model. See `references/lmstudio-empty-stream-reasoning.md`.
...
**Symptom**: Hermes Desktop works in terminal but not from app menu.
**Cause**: PATH mismatch — GUI launcher missing NVM/pyenv/conda paths.
**Fix**: Hardcode PATH in `~/.local/bin/hermes-desktop`. Full guide: `skill_view(name="hermes-agent", file_path="references/desktop-launcher-path-fix.md")`.

### LM Studio empty stream
**Symptom**: "API call failed after 3 retries: Provider returned an empty stream"
**Cause**: Reasoning models (Qwen3.5, o1/o3) send `reasoning_content` before `content`.
**Fix**: Disable reasoning in LM Studio or use non-reasoning model. Full guide: `skill_view(name="hermes-agent", file_path="references/lmstudio-empty-stream-reasoning.md")`.
