...
- **LM Studio Endpoint Port Quirks** — LM Studio can bind to different ports depending on setup (commonly `3000` or `1234`). Make sure to check the active port via `/home/yan/.lmstudio/bin/lms status` or your platform-equivalent before configuring `base_url`.
- **Model Load Guardrails in Local Servers** — When loading models on local servers (like LM Studio or llama.cpp) with large context windows (e.g. 200,000 tokens), the server may reject the load with a system resource warning or crash with a memory allocation failure (OutOfMemory / CUDA allocation failed) if the physical RAM/VRAM capacity is exceeded. **Fix:** Try using `--gpu off` (CPU execution only) to bypass small VRAM limits and incrementally reduce the context length (e.g., from 200k to 128k or 64k) until the model successfully loads within physical limits. Use `lms load <model-key> --context-length <length> --gpu off -y` or `--estimate-only` to verify first.
